class DbProxy(object):
    ...
    def open(self):
        conn_str = "dbname='{0}' user='{1}' host='{2}' port={3} password='{4}'"
        self.conn = self.dbapi.connect(
            conn_str.format(self.dbname, self.dbuser, self.dbhost,
                            self.dbport, self.dbpass))
...
def create(dbtype, dbhost, dbport, dbname, dbuser, dbpass, schema):
    ...
    if dbtype.lower() == 'postgresql':
        ...
        return dbproxymod.Postgresql(dbapi, dbtype, dbhost, dbport, dbname,
                                     dbuser, dbpass, schema)
    else:
        raise NotImplementedError()
